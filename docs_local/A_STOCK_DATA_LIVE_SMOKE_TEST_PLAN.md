...
## 14. Phase D Direct Function Smoke
...
for symbol in ("600519.SH", "300750.SZ"):
    for statement in ("income", "balance", "cashflow"):
        ...
        norm = normalize_a_stock_financials_result(
            raw,
            symbol,
...
* No `local_reports` generated by this direct smoke.
* Feature flag remains default off.

## 15. Phase E Controlled Direct Tool Fallback Smoke
...
* Calls `FinancialStatementsTool().execute(...)` directly.
* Sets `VIBE_TRADING_ENABLE_A_STOCK_DATA_ADAPTER=1` only for the script process.
* Forces primary provider failure with:

```python
{"ok": False, "error": "forced_primary_failure_for_smoke", "data": []}
```

Positive results:

| Symbol | Statement | Provider | Source | Rows | Latest Date | Result |
| --- | --- | --- | --- | ---: | --- | --- |
| `600519.SH` | `income` | `a_stock_data` | `sina_financial_report` | 8 | `2026-03-31` | pass |
| `300750.SZ` | `income` | `a_stock_data` | `sina_financial_report` | 8 | `2026-03-31` | pass |
| `600519.SH` | `balance` | `a_stock_data` | `sina_financial_report` | 8 | `2026-03-31` | pass |
| `600519.SH` | `cashflow` | `a_stock_data` | `sina_financial_report` | 8 | `2026-03-31` | pass |

Negative results:

| Symbol | Statement | Reason | Fallback Called | Result |
| --- | --- | --- | --- | --- |
| `510300.SH` | `income` | `etf_not_eligible_for_company_financials` | false | pass |
| `QQQ.US` | `income` | `not_a_share_market` | false | pass |
| `000001.SH` | `income` | `index_not_eligible_for_company_financials` | false | pass |
...
* No LLM key needed.
* No provider-chain or loader change.
* No local JSON report was generated in this run.
* Feature flag remains default off.
